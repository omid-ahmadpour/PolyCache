...
using Nito.AsyncEx;
using PolyCache.ComponentModel;
...
namespace PolyCache.Cache
{
    ...
    public partial class DistributedCacheManager : CacheKeyService, ILocker, IStaticCacheManager
    {
        ...
        public async Task<T> GetWithExpireTimeAsync<T>(CacheKey key, int expireTime, Func<Task<T>> acquire)
        {
            //little performance workaround here:
            //we use "PerRequestCache" to cache a loaded object in memory for the current HTTP request.
            //this way we won't connect to Redis server many times per HTTP request (e.g. each time to load a locale or setting)
            ...
            if (result != null)
                await SetAsync(key, result, expireTime);
        ...
        protected virtual async Task SetAsync(CacheKey key, object data, int cacheTime)
        {
            if ((key?.CacheTime ?? 0) <= 0 || data == null)
                return;
            ...
            await _distributedCache.SetStringAsync(key.Key, JsonConvert.SerializeObject(data), options);
            _perRequestCache.Set(key.Key, data);
